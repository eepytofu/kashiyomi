...
- First run offers the Core (about 207 MB installed) and Full (about 343 MB installed) Japanese dictionaries, with Core selected by default. Both editions can remain installed, but only one is active at a time; Kashiyomi settings can switch or remove either edition.
...
`npm run fetch-dict` downloads SudachiDict Core, about 207 MB installed, so a development build has one without going through the first-run download. Pass `full` to fetch the roughly 343 MB Full edition. Runtime installs use the embedded, reviewed release manifest described above; the development fetch is not part of the plugin package. `npm run export-pinyin` prepares Pinyin Pro's complete dictionary, and `npm run export-jmdict` builds the roughly 7 MB JMdict reading table used to fill readings the analyzer cannot supply.
